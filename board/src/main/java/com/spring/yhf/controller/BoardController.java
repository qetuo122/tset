...
public class BoardController {
	...
	public @ResponseBody void cart(HttpServletRequest request, @RequestParam(value="list", required=false) String[] arr) {
		...
		if(arr != null) {
			for(int i = 0; i < arr.length; i++) {
				BoardVO boardVO = new BoardVO(); // boardVO 객체를 계속해서 새로 생성하지 않으면 기존의 boardVO데이터에 계속 덮어씌우기밖에 안함.
				System.out.println("배열로 받아온 파라미터" + arr[i]);
				boardVO.setId(Integer.parseInt(arr[i].split("- ")[0]));
				boardVO.setTitle(arr[i].split("- ")[1]);
				boardVO.setImg(arr[i].split("- ")[2]);
				boardVO.setCon(arr[i].split("- ")[3]);
				
				System.out.println("데이터가 저장되기 전 배열" + list);
				list.add(boardVO);
				System.out.println("데이터가 저장된 후  배열" + list);
			}
		} 
